...
import { stockistScope, getDepotSchemesData, pickStockist, type SchemeClaimRow } from "@/lib/depot/data";
...
export default async function DepotSchemesPage({
  ...
  if (scope.length === 0) {
    const t = await getT();
    return <Notice title={t("Schemes")}>{t("You aren't mapped to a stockist yet — ask Central Admin.")}</Notice>;
  }
