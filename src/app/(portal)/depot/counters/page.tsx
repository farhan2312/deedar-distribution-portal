...
export default async function DepotCountersPage({
  ...
  if (!canAccess(user, "dealer")) {
    const t = await getT();
    return <Notice title={t("Counters")}>{t("You don't have Depot access.")}</Notice>;
  }

  const scope = await depotScope(user);
  if (scope.length === 0) {
    const t = await getT();
    return <Notice title={t("Counters")}>{t("You aren't mapped to a depot yet — ask Central Admin.")}</Notice>;
  }
